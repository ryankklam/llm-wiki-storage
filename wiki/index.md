# Wiki Index

## 概览
- [[overview]] - 整体综述

## 来源
- **2026-08-08** [一只尺就能骗过你大脑实验-橡胶手错觉]([[2026-08-08-一只尺就能骗过你大脑实验-橡胶手错觉]]) 📕小红书
- **2026-08-06** [我现在给 agent 补上下文，只做这四层]([[2026-08-06-我现在给agent补上下文只做这四层]]) 📕小红书
- **2026-08-01** [生产级Agent 缺的不是Prompt，Context]([[2026-08-01-生产级Agent缺的不是PromptContext]]) 📕小红书
- **2026-07-24** [Agent成本优化：如何让同样的任务少花钱]([[2026-07-24-Agent成本优化如何让同样的任务少花钱]]) 📕小红书
- **2026-07-03** [面试问密码怎么存？答MD5直接被淘汰]([[2026-07-03-面试问密码怎么存答MD5直接被淘汰]]) 📕小红书
- **2026-06-24** [大模型面试精讲 Loop Engineering]([[2026-06-24-大模型面试精讲LoopEngineering]]) 📕小红书
- **2026-06-20** [面试官：商品库存存在Redis里，怎么保证缓存和数据库的数据一致性？]([[2026-06-20-Redis缓存与数据库一致性]]) 📕小红书
- **2026-06-19** [Agent记忆管理：别只答向量库加RAG]([[2026-06-19-Agent记忆管理别只答向量库加RAG]]) 📕小红书
- **2026-06-17** [Agent Skill过多？4招提升命中]([[2026-06-17-AgentSkill过多4招提升命中]]) 📕小红书
- **2026-06-17** [AI 编程交付企业级项目 Spec-Kit 是必学技术]([[2026-06-17-AI编程交付企业级项目SpecKit]]) 📕小红书
- **2026-06-14** [Loop Engineering：做设计循环的人]([[2026-06-14-LoopEngineering做设计循环的人]]) 📕小红书
- **2026-06-12** [企业AI项目rules红线：5条硬规矩+6个写法门道]([[2026-06-12-企业AI项目rules红线]]) 📕小红书
- **2026-06-11** [瑞幸为什么宁愿直播卖券，也不直接降价？]([[2026-06-11-瑞幸为什么宁愿直播卖券]]) 🎵抖音
- **2026-05-10** [skill实战: 从0到1写一个你自己的skill]([[2026-05-10-skill实战- 从0到1写一个你自己的skill]])
- **2026-05-25** [Pi Agent比Codex更适合普通人的AI工具]([[2026-05-25-Pi Agent比Codex更适合普通人的AI工具]])
- **2026-05-28** [如何在有限上下文窗口内放入关键内容]([[2026-05-28-如何在有限上下文窗口内放入关键内容]])
- **2026-05-31** [FDE脚下的地基Ontology]([[2026-05-31-FDE脚下的地基Ontology]]) 🎵抖音
- **2026-05-31** [AI Agent四种范式对比]([[2026-05-31-AI Agent四种范式对比]]) 📕小红书
- **2026-06-05** [Anthropic 数据 Agent 95% 准确率背后②]([[2026-06-05-Anthropic数据Agent95%准确率背后②]]) 📕小红书
- **2026-06-09** [Agent Loop构建、步骤、实战之完整指南]([[2026-06-09-AgentLoop构建步骤实战指南]]) 📕小红书


## 实体
<!-- 人物、组织、产品等 -->
- [[Claude]] - Anthropic 开发的大语言模型
- [[Anthropic]] - AI 安全研究公司
- [[Palantir]] - 大数据分析公司，Foundry平台

## 概念
- [[rules.md]] - 立给 AI 的项目规则文件
- [[CLAUDE.md]] - Claude Code 的项目规则文件
- [[AI项目规范]] - 企业级 AI 编程项目规范体系
- [[Spec-Kit]] - Anthropic 官方开源的 Spec 驱动开发框架
- [[企业级AI项目]] - 使用 AI 编程工具进行企业级软件开发
- [[AI编程交付]] - 使用 AI 编程工具完成软件项目全流程交付
- [[Constitution]] - Spec-Kit 中的项目宪法机制
- [[立宪法]] - Spec-Kit 第一步核心命令
- [[主动追问]] - Spec-Kit 的 Clarify 可选关卡
- [[一致性检查]] - Spec-Kit 的 Verify 可选关卡
- [[OpenSpec]] - 轻量级 Spec 驱动开发框架
- [[Gate]] - Spec-Kit 的门控机制
- [[Prompt工程规范]] - Prompt 编写最佳实践体系
- [[System Prompt]] - 系统级提示词
- [[代码规范]] - AI 时代的代码约束语言
- [[红线规则]] - AI 绝对不能触碰的边界
- [[工业级]] - 稳定可控的 AI 项目交付标准
- [[范式]] - AI 编程的系统化方法论
- [[项目管理]] - AI 时代的项目管理
- [[版本控制]] - 技术栈版本约束与 Git 管理
- [[代码审查]] - AI 生成代码的审查机制
- [[Code Review]] - AI 生成代码的审查机制
- [[自动化]] - AI 按规范自动执行开发任务
- [[CI/CD]] - 持续集成/持续部署
- [[测试]] - AI 生成代码的验证环节
- [[部署]] - AI 生成代码的发布过程
- [[文档]] - 写给 AI 的语言书
- [[API]] - AI 项目中的接口规范
- [[JSON]] - 数据交换格式
- [[XML]] - 结构化数据标记语言
- [[Markdown]] - 规则文件的标准格式
- [[Prompt Engineering]] - 提示工程
- [[Context Window]] - 上下文窗口
- [[Token]] - 大模型处理文本的基本单位
- [[Function Calling]] - LLM 调用外部函数的能力
- [[Tool Use]] - Agent 基础能力层
- [[RAG]] - 检索增强生成
- [[Git]] - 版本控制工具
- [[AI Agent]] - 自主执行任务的 AI 系统
- [[LLM]] - 大语言模型
- [[Ontology]] - 本体论，语义数据模型
- [[FDE]] - Foundry Data Engine
- [[数字孪生]] - 现实世界的数字映射
- [[语义建模]] - 用语义方式定义数据结构
- [[Pi Agent]] - 极简底座的日常任务Agent
- [[Skill]] - Agent的能力扩展单元
- [[Skill路由]] - Agent Skill选择机制
- [[Skill Tree]] - Skill分层组织结构
- [[分层路由]] - 分层缩小搜索空间的路由策略
- [[负样本]] - Skill描述中的When Not to Use
- [[召回重排]] - 两阶段检索策略
- [[渐进式加载]] - Progressive Disclosure，按需加载Skill内容
- [[Skill Router]] - 技能路由器组件
- [[语义匹配]] - 基于语义理解的匹配技术
- [[Coding Agent vs 日常任务Agent]] - Agent的两大分类
- [[Claude Code]] - Coding Agent代表产品
- [[Cursor]] - AI 编程编辑器
- [[Skill Creator]] - Skill创建工具
- [[Skill Store]] - Skill商店
- [[按需加载]] - Skill的安装策略
- [[GitHub]] - 代码托管平台
- [[Context Window优化]] - 上下文窗口的优先级管理策略
- [[短期记忆管理]] - 当前会话的记忆处理方法
- [[长期记忆管理]] - 跨会话的稳定偏好存储
- [[Sliding Window]] - 滑动窗口策略
- [[摘要压缩]] - 将长内容压缩为关键信息
- [[脏记忆防护]] - 防止错误/过期记忆污染上下文
- [[Agent记忆管理]] - 生产级Agent记忆完整系统
- [[显式读写]] - Agent主动调用记忆工具
- [[隐式读写]] - 被动全量历史塞进Prompt
- [[状态管理三件套]] - Ledger+View+Policy记忆状态管理
- [[双时间机制]] - Valid Time+Transaction Time解决时间盲区
- [[程序性经验]] - 把成功轨迹固化为可复用Skill
- [[Agent范式]] - AI Agent架构范式总览
- [[ReAct]] - 推理与行动交织的范式
- [[Plan-and-Execute]] - 先规划再执行的范式
- [[Multi-Agent]] - 多智能体协作范式
- [[AutoGen]] - Microsoft多Agent框架
- [[CrewAI]] - 角色扮演式多Agent框架
- [[Function Calling]] - LLM调用外部函数的能力
- [[Data Agent]] - 利用LLM进行自助业务分析的智能系统
- [[Anthropic数据Agent架构]] - Anthropic四层数据分析Agent系统
- [[数据Agent四层架构]] - 数据基础、可信来源、Skills、验证
- [[语义层]] - 公司批准过的指标接口
- [[血缘图]] - 数据从源头到展示的完整流转路径
- [[消融实验]] - 评估系统组件价值的实验方法
- [[对抗审查]] - 子Agent专门挑错的验证机制
- [[来源追溯]] - 答案可信度标注机制
- [[静默失败]] - 答案错了但看起来合理的失败模式
- [[Agent Loop]] - Agent的核心循环运行机制
- [[Loop Engineering]] - 为Agent设计循环系统的工程方法论
- [[Harness Engineering]] - 为Agent提供运行环境的工程实践
- [[Agentic Loop]] - 强调自主性的Agent循环
- [[Supervisor]] - Agent Loop中的监督代理
- [[Guardrails]] - Agent Loop中的安全护栏机制
- [[Open Loop]] - 开放循环模式
- [[Closed Loop]] - 闭环模式
- [[Sub-Agents]] - Loop中的子代理，Maker和Checker分离
- [[Automations]] - Loop的自动化触发机制
- [[Worktrees]] - Git工作树隔离
- [[State]] - Loop的状态记忆系统
- [[自动循环系统]] - Loop Engineering的中文表述
- [[价格歧视]] - 让不同支付意愿的人支付不同价格
- [[锚定效应]] - 消费者对商品正常价格的基准认知
- [[沉没成本]] - 已发生且不可收回的成本对决策的影响
- [[消费者心理]] - 购买决策中的心理活动规律
- [[定价策略]] - 企业为实现经营目标的价格决策方案
- [[直播卖券]] - 直播间销售优惠券的精细化营销模式
- [[品牌营销]] - 建立和维护品牌形象与价值的过程
- [[行为经济学]] - 经济学与心理学的交叉学科
- [[缓存一致性]] - Redis缓存与MySQL数据库之间的数据同步问题
- [[延迟双删]] - 先删缓存→更新DB→延迟再删缓存的策略
- [[缓存穿透]] - 查询不存在的数据穿透到数据库
- [[缓存击穿]] - 热点Key过期瞬间大量请求打到数据库
- [[缓存雪崩]] - 大量缓存Key同时过期或缓存服务宕机
- [[Cache Aside]] - 最常用的缓存策略模式
- [[Canal]] - 阿里巴巴开源的MySQL binlog增量订阅组件
- [[消息队列]] - 异步通信机制，用于解耦和削峰填谷
- [[分布式锁]] - 分布式系统中控制共享资源互斥访问的机制
- [[最终一致性]] - 允许短暂不一致但最终达到一致的模型
- [[强一致性]] - 任何时刻所有节点数据完全一致
- [[Redis]] - 开源内存数据结构存储系统
- [[MySQL]] - 最流行的开源关系型数据库管理系统
- [[后端面试]] - 后端开发技术面试考察方向
- [[密码安全]] - 密码全生命周期安全保护措施
- [[MD5]] - 已淘汰的128位哈希算法
- [[BCrypt]] - 业界主流的慢哈希密码算法
- [[加盐]] - 密码哈希随机化技术
- [[彩虹表]] - 预计算哈希破解技术
- [[暴力破解]] - 穷举式密码攻击方法
- [[慢哈希]] - 专门用于密码存储的慢速哈希算法
- [[哈希算法]] - 单向指纹计算算法
- [[Agent成本优化]] - 在保证质量前提下降低Agent任务总成本
- [[Token成本]] - Agent运行中的Token消耗费用与优化
- [[模型路由]] - 为请求选择最优后端LLM的调度系统
- [[缓存策略]] - 降低Agent循环中重复Token传输开销
- [[批处理优化]] - 确定性高操作下放给高性价比模型
- [[Context Layer]] - 将企业知识转化为AI可用上下文的系统层
- [[Company Brain]] - 可复用的企业级知识中枢
- [[Compounding Learning Loop]] - Agent Trace回流持续修正Skills与Context的闭环
- [[GitHub for Context]] - Context像代码一样进行版本管理、依赖管理、质量管理
- [[四层上下文架构]] - Agent上下文分层实践：项目地图+按需知识+任务现场+长期记忆
- [[AGENTS.md]] - 精简后的项目地图，只回答三个核心问题
- [[按需知识树]] - 知识分层组织，Agent按任务需要查询对应分支
- [[任务现场]] - 每次任务开始时动态组装Issue、代码、口径、时间、环境、日志
- [[橡胶手错觉]] - 经典身体所有权错觉实验（Botvinick & Cohen, 1998）
- [[身体所有权]] - 大脑对"这是我的身体"的动态感知
- [[镜子疗法]] - 利用视觉反馈治疗幻肢痛的临床技术（Ramachandran, 1992）
- [[多感官整合]] - 视觉、触觉、本体感觉的协同整合机制

## 主题
<!-- 综合分析、比较等 -->

## 经验
<!-- 解决问题的经验 -->
